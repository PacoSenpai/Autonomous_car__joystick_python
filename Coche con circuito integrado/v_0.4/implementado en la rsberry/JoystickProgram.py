...
#motor.rightMotor(100,1)  
#motor.leftMotor(100,1) 
...
def joystick (vector): # joystick
	...
	if mouse_inscreen == 1 and calculate_module(joyX,joyY) <= 100:
		...
		canvas.create_text(vector.x,vector.y+10, text = (int (joyX),int(joyY),int(calculate_module(int (joyX),int(joyY))) ), fill = 'green')
		...
		if joyX > 0 and joyY <= 0: # cuadrante 4
			...
			rightV = int((100 - math.cos(angulo)*100) * modulo / 100)
			fwd = 0
		control_screen.create_text(130,15, text = ("Lv = ",int (leftV),"Rv = ",int(rightV),"modulo = ",int(calculate_module(int (joyX),int(joyY))) ), fill = 'green')

		motor.rightMotor(rightV,fwd)  
		motor.leftMotor(leftV,fwd)  
		#print (leftV,rightV)
