...
def joystick (vector): # joystick
        ...
        if mouse_inscreen == 1 and calculate_module(joyX,joyY) < 100:
                ...
                if joyX < 0 and joyY <= 0: #cuadrante 3 atras i
                        leftV = int((100 - math.cos(angulo)*100) * modulo/100)
                        rightV = int(modulo)
...
def velocidad ():
        ...
        if joyX > 0 and joyY > 0: # cuadrante 1
                leftV = int((joyX * modulo)/10000)
                rigthV = int((100-joyX) * modulo)/10000
        
        if joyX < 0 and joyY > 0:# cuadrante 2
                leftV = int((100-joyX * modulo)/10000)
                rigthV = int((joyX) * modulo)/10000
...
#pwm_L = GPIO.PWM(12,100)
#pwm_R = GPIO.PWM(9,100)
